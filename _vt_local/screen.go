...
// NOTE: In newer ultraviolet versions, Touched was moved from Buffer to
// RenderBuffer. We return nil here since we don't use dirty-line tracking.
...
// scrolled past the top margin are captured via ScrollOut callback before
// being deleted. This is equivalent to [ansi.SU] which moves the cursor to
// the top margin and performs a [ansi.DL] operation.
func (s *Screen) ScrollUp(n int) {
	// スクロールアウトする行を callback で通知（DeleteLine で消える前に）
	if s.cb != nil && s.cb.ScrollOut != nil {
		topY := s.scroll.Min.Y
		for i := 0; i < n && topY+i < s.scroll.Max.Y; i++ {
			line := s.buf.Line(topY + i)
			s.cb.ScrollOut(line.String(), line.Render())
		}
	}
